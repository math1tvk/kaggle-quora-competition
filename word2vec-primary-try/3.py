...
def paired_dist_vecOfvec(short_q_v,long_q_v,short_q_words,long_q_words): #vector of vectors, and vector of words
    ...
    return q1_q2_words_dist

def avg_vecOfvec(vecOfvec, num_features):
    ...
    for word_dst in vecOfvec:
        featureVec = np.add(featureVec,word_dst)
    #
    # Divide the result by the number of words to get the average
    if nwords == 0:
        return featureVec
...
def main():
    ...
    clean_train_short_q = [x if len(x) < len(y) else y for x,y in zip(clean_train_question1,clean_train_question2)]
    clean_train_long_q = [x if len(x) > len(y) else y for x,y in zip(clean_train_question1,clean_train_question1)]
    ...
    train_distance_q1_q2 = [avg_vecOfvec(paired_dist_vecOfvec(short_q_v,long_q_v,short_q_words,long_q_words),num_features) for short_q_v,long_q_v,short_q_words,long_q_words in zip(vectorsOfvector_train_short_q,vectorsOfvector_train_long_q, clean_train_short_q, clean_train_long_q)]
